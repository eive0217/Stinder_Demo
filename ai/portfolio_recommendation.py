...
from logic.schemas import Portfolio
...
def recommend_portfolio(p,stocks):
    if api_enabled():
        try:
            result = structured_call(SYSTEM_PROMPT,{'USER PROFILE':p.model_dump(exclude={'summary'}),'STOCK DATA':stocks,'risk_limit':risk_limit(p)},Portfolio)
            return validate_selection(result,p,stocks),'ai'
        except RECOVERABLE as exc:
            report_fallback('portfolio',exc)
    return fallback_portfolio(p,stocks),'fallback'
