...
let current=0,busy=false,run=0,result=null,selectedPlan=0;
...
async function analyze(){
  ...
  try{
    ...
    const portfolio=logic.options(p,data.stocks);await wait(650);if(token!==run)return;
    result={profile:p,portfolio};selectedPlan=0;renderResult();bar.value=100;title.textContent='맞춤 포트폴리오 생성 완료';second.textContent='✓ 맞춤 포트폴리오 생성 완료';second.className='done';
...
function renderResult(){
  const p=result.profile,pf=result.portfolio.portfolios[selectedPlan],rows=pf.recommendations,esc=escapeHTML;
  ...
  document.querySelector('#view-result').innerHTML=`<div class="row result-heading"><div><span class="eyebrow">나의 투자 성향</span><h1>투자 성향 분석 결과</h1></div><a class="secondary" href="#test">다시 테스트하기 ↗</a></div><section class="card"><h2>포트폴리오 추천안 3개</h2><p>각 안은 별도의 선택지입니다. 한 안의 종목 3개에 투자 금액 전액을 배분합니다.</p><nav class="plan-options" aria-label="추천안 선택">${result.portfolio.portfolios.map((option,i)=>`<button type="button" class="secondary" data-plan="${i}" aria-pressed="${i===selectedPlan}">추천안 ${['A','B','C'][i]} · ${option.recommendations.map(r=>esc(r.ticker)).join(' / ')}</button>`).join('')}</nav></section><div class="result-grid"><section class="profile card"><span class="pill">규칙 기반 성향 분석</span><h2>${esc(data.types[p.investor_type])}</h2><p>${esc(p.summary)}</p><div class="scores">${scores}</div></section><section class="card"><div class="row"><span class="eyebrow">모델 포트폴리오</span><span class="pill">규칙 기반 추천</span></div><h2>추천안 ${['A','B','C'][selectedPlan]} · 종목과 비중</h2><p class="muted">가상 종목 데이터 기반 · 총 비중 100%</p><div class="allocation-bar" role="img" aria-label="${esc(rows.map(r=>`${r.ticker} ${r.allocation}%`).join(', '))}">${rows.map(r=>`<div style="width:${r.allocation}%">${r.allocation}%</div>`).join('')}</div><div class="legend">${rows.map((r,i)=>`<span><i class="color-${i+1}"></i>${esc(r.ticker)} <b>${r.allocation}%</b></span>`).join('')}</div><div class="amount-box"><label for="investment">투자 예정 금액</label><div class="amount-input"><input id="investment" type="number" min="0" max="1000000000000" step="1" value="1000000" inputmode="numeric"><span>원</span></div><p id="amount-error" class="error" role="alert"></p><div class="amounts">${rows.map(r=>`<div><span>${esc(r.ticker)}</span><strong data-allocation="${r.allocation}"></strong></div>`).join('')}</div><p class="small muted">비중에 따른 예산 예시이며 실제 주문 수량은 아닙니다.</p></div></section></div><div class="section-heading"><h2>나와 잘 맞는 세 종목</h2><span class="muted small">모든 수치와 애널리스트 의견은 가상 예시</span></div><section class="stock-grid">${cards}</section><section class="card explanation"><span class="eyebrow">포트폴리오 해설</span><h2>이 포트폴리오는 왜 이렇게 구성되었나요?</h2><p>${esc(pf.portfolio_summary)}</p><p class="small muted">변동성과 최대 낙폭 위험은 실제 백분율이 아닌 0~10의 가상 위험 지표입니다. 매칭 점수는 수익 확률이 아닙니다.</p></section>`;
  document.querySelectorAll('[data-plan]').forEach(button=>button.onclick=()=>{
    const amount=document.querySelector('#investment').value;
    selectedPlan=Number(button.dataset.plan);renderResult();
    document.querySelector('#investment').value=amount;calculate();
  });
...
function calculate(){
  ...
  try{if(input.value==='')throw Error('투자 예정 금액을 입력해주세요.');const values=logic.amounts(Number(input.value),result.portfolio.portfolios[selectedPlan].recommendations.map(r=>r.allocation));cells.forEach((c,i)=>c.textContent=values[i].toLocaleString('ko-KR')+'원');document.querySelector('#amount-error').textContent='';}
